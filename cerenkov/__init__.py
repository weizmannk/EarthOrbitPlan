...
class CerenkovBackground:
    ...
    def evaluate(self):
        ...
        for i in range(Nn):
            ...
            intg_i = gEE * Fm * fCi
            Lnorm = 2 * np.pi / 137 / rho / (1e-8 * Lam[i]) ** 2 * cs_intg(1) # count/cm^2/s/cm(wave)
            ...
            IC1mu_array[i] = L1mu_array[i] / (2 * np.pi * n[i] ** 2)  #intensity at 1 micron


        # Convert steradians to arcsec^2 and photon flux to energy flux
        RAD = 180.0 / np.pi  # Conversion factor from radians to degrees
        int_aa = IC1mu_array / 1e4 / ((RAD * 3600) ** 2)  # Convert intensity to arcseconds squared
        ...
        Flux_mJy = int_aa * planck_h * Freq_Hz * (speed_c * 1e8 / (Freq_Hz ** 2)) / 1e-26  # Flux in mJy
        ...
        Int_m4opt = Int_m4opt.to(u.count *  u.cm**-2 * u.s**-1 *  u.sr**-1  * u.Angstrom**-1)
         # Result dictionary
        result = {
            "Lam": Lam,
            "Int": IC1mu_array,

# from https://github.com/m4opt/m4opt-paper
import numpy as np
import synphot
...
def get_detection_probability_known_position(plan, event_row, plan_args):
    """
    Estimate the detection probability of an astronomical transient at a known position
    given an observation plan.

    This function calculates the probability that a source, located at a precise sky
    position and distance (for example, a known kilonova counterpart), would be
    detected by the planned observations. The probability is evaluated by comparing
    the source's expected absolute magnitude distribution to the limiting magnitude
    achieved in any field that covers the true position.

    Parameters
    ----------
    plan : astropy.table.Table
        Table of scheduled observations, including pointing coordinates, times, and durations.
    event_row : dict or row-like
        Event information containing at least 'longitude', 'latitude' (in radians), and 'distance' (in Mpc).
    plan_args : dict
    ...
    -------
    probability : float
        Probability that the transient would be detected by the observation plan,
        given its true position and distance.
    """
    if len(plan) == 0:
        return 0
...
def get_detection_probability_unknown_position(plan, skymap_moc, plan_args):
    ...
    described by a gravitational-wave probability sky map (skymap).
    ...
    plan : astropy.table.Table
        Table of scheduled observations, including pointing coordinates, times, and durations.
    skymap_moc : mocpy.MOC or equivalent
        Gravitational-wave localization skymap (in MOC/HEALPix format), encoding the sky probability

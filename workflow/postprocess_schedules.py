...
This script post-processes observation plans generated by the M4OPT scheduler. It computes
detection probabilities for each gravitational-wave event and extracts key optimization metrics,
such as objective value, solution status, and runtime.

The script supports both command-line arguments and configuration via a `.ini` file.

Functionality
-------------
- Loads a summary table of gravitational-wave events.
- For each event, reads the corresponding observation schedule.
- Computes the detection probability assuming a known source position.
- Extracts relevant optimization results from the observation plan metadata.
- Outputs an enhanced event table with new metrics for further analysis.
...
Parameters (via CLI or config)
------------------------------
--data-dir       Directory containing schedule and event files (default: data)
--event-table    Filename of the summary table with event information (default: observing-scenarios.ecsv)
--output-file    Output filename for the results table (default: events.ecsv)
--sched-dir      Directory containing observation schedule files (default: schedules)
...
- astropy
- ligo.skymap
- detection_probability module (with get_detection_probability_known_position)
